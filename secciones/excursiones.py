import streamlit as st  # Corregido: faltaba el 'as'
import os
...
def render_excursiones(destino):
    ...
    if destino == "Villa Carlos Paz":
        ...
        # 2. EL FUERTE DE OBLIGADO
        st.markdown('<div class="excursion-card">', unsafe_allow_html=True)
        if os.path.exists("assets/sanpedroexc2.jpg"):
            st.image("assets/sanpedroexc2.jpg", use_container_width=True)
        st.markdown("""
            <div class="excursion-content">
                <div class="excursion-title">⚔️ Vuelta de Obligado</div>
                <div class="excursion-desc">Recorrido histórico por el sitio de la mítica batalla a orillas del Paraná.</div>
                <div class="excursion-tag">Historia • Paisaje</div>
            </div>
        </div>""", unsafe_allow_html=True)
