...
def render_tarifas(destino):
    ...
    # CSS para tarjetas: Número e ícono a la par, texto abajo
    st.markdown("""
        <style>
        .plan-card-click {
    ...
    if os.path.exists(path_tarifas):
        ...
        # Widgets de monto 3D en escala de grises
        ...
        with col_monto:
            st.markdown(f"""<div class="widget-3d-inner"><p class='widget-title'>Monto {cuota_sel}</p><p class='widget-value'>${val_c:,.0f}</p></div>""", unsafe_allow_html=True)

        with col_cash:
            st.markdown(f"""<div class="widget-3d-inner"><p class='widget-title'>💎 Efectivo (10% OFF)</p><p class='widget-value' style='color: #495057;'>${val_cont * 0.9:,.0f}</p></div>""", unsafe_allow_html=True)

        st.info("Nota: Se pueden pautar otros planes según la necesidad de cada familia[cite: 120, 250, 391].")
    else:
        st.error(f"No se encontró el archivo en {path_tarifas}")
